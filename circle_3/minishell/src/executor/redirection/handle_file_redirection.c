...
/*   Updated: 2025/12/11 12:41:19 by wtang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 文件重定向处理主流程
// =============================
// 打开文件并根据需要重定向到指定文件描述符
// 打开 info.file，并根据 info.should_dup 决定是否 dup2 到目标 fd。
// 失败时输出错误信息。
...
// 根据重定向类型分发处理：
// 1. 输入重定向调用 handle_redir_in
// 2. 输出重定向调用 handle_redir_out
// 3. 追加重定向调用 handle_redir_append
// 4. 其他类型报错
