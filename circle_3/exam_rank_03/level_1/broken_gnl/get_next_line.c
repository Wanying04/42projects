...
char *get_next_line(int fd)
{
  ...
  static int pos = 0; // 记录缓冲区中已处理数据的位置!!!
  char *ret = NULL; // 最终要返回的一行字符串
  char *tmp = ft_strchr(b + pos, '\n'); // 在当前未处理的数据中查找换行符,从 b + pos 开始!!!

  while(!tmp) // 没有找到换行符时进入循环
  {
	if (b[pos] != '\0') // 上一轮读取的数据没有换行符，但还有剩余字符没处理完!!!
	{
		printf("DAADFDF\n");
		int len = ft_strlen(b + pos); // 计算当前缓冲区中剩余数据的长度!!!
		if (len > 0) // 如果有数据!!!
		{
			if (!str_append_str(&ret, b + pos)) // 将剩余数据追加到 ret!!!
			{
